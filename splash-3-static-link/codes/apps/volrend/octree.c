...
*/
...
#ifndef SERIAL_PREPROC
  {
	...
	for (i = 0; i < (num_nodes) - 1; i++) {
		Error = pthread_create(&__tid__[__threads__++], NULL, (void * (*)(void *))(Compute_Base), NULL);
		if (Error != 0) {
...
*/
...
#ifndef SERIAL_PREPROC
  {
	...
	pthread_mutex_lock(&__intern__);
	for (i = 0; i < (num_nodes) - 1; i++) {
		Error = pthread_create(&__tid__[__threads__++], NULL, (void * (*)(void *))(Or_Neighbors_In_Base), NULL);
		if (Error != 0) {
			printf("Error in pthread_create().\n");
			exit(-1);
		}
